...
\FloatBarrier
Looking at table ~\ref{tab:query_accuracy} along with figure ~\ref{fig:accuracy_vs_query_count}, you can spot how query numbers tie into accuracy. More timing data tends to lift classification quality. Performance crept up from 80.00\% using one query, reaching 87.5\% once ten entered the mix. Though small gains, they follow a steady climb.

One reason for smoother results with larger query counts? Gathering multiple time samples smooths out random hiccups in how fast things run. Odd little jumps seen partway through could come from background task shifts or tiny delays deep in the machine while predictions happen

Surprisingly clear outcomes emerge when black-box data is captured again and again - each new recording sharpens the ability to pin down a model. This steady gain hints that timing patterns, paired with smart number crunching, can indeed expose hidden identities behind software systems.
% ---------------- DISCUSSION -------------
